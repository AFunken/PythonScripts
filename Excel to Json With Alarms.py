...
def excel_to_json(excel_file, udt_name, output_file='output.json'):
    ...
    - alarmMode: Alarm mode (e.g., 'WhenTrue', 'AboveValue', 'BelowValue') - optional
    - alarmName: Alarm name (e.g., 'Alarm', 'High Alarm') - optional
    """
    try:
        # Read Excel file
        ...
        for _, row in df.iterrows():
            if pd.notna(row.get('name')):  # Only process rows with a name
                ...
                # Add alarms if provided (alarms come first)
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        ...
        print("  - alarmMode (optional): Alarm mode (WhenTrue, AboveValue, etc.)")
        print("  - alarmName (optional): Alarm name (default: Alarm)")
        sys.exit(1)
